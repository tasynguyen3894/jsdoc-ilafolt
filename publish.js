...
function buildHeaderNav() {
    ...
    let nav = []
    if (conf.navigates !== undefined) { // need check array
        conf.navigates.forEach(function (item) {
            if (item.text && item.url) {
                let _blank = item._blank ? 'target="_blank"' : '';
                nav.push(`<a href="${item.url.toString()}" ${_blank}>${item.text.toString()}</a>`)
            }
        });
    }
    return nav;
            ...
            if (!hasOwnProp.call(itemsSeen, item.longname)) {
                ...
                if (methods.length) {
                    ...
                    methods.sort(function (m, n) {
                        return m.meta.lineno > n.meta.lineno ? 1 : -1;
                    });
...
function buildNavLink(linkClass, linkContent) {
    return {
        el: {
            content: linkContent,
            class: "nav-link nav-' + linkClass + '-link"
        }
    }
}
...
exports.publish = function (taffyData, opts, tutorials) {
    ...
    view.headerNav = buildHeaderNav(members);
